...
class LendingRepay(LoanDataLendingBasic):
    task_name = 'lending_repay'

    table_name = 'lending_repay'
    ...
    def build_query_sql(self):
        ...
        sql_path = os.path.join(dags_folder, 'lending/lending_asset/lending_repay.sql')
        sql = read_file(sql_path)
        sql = sql.replace('{{date_filter}}', self.date_filter)
        print('lending repay build_query_sql query_string', sql)
        return sql
    ...
    def anomaly_sql(self):
        ...
        return """
                WITH daily AS 
                    ...
                    FROM `xed-project-237404.{data_base}.lending_repay`
                    WHERE day > {date15ago}
