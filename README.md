...
Use airflow DAGs to obtain data from various data sources and upload it to bigquery
...
1. Create a GCS bucket to hold export files Create a bucket to store temporary data:
...
2. Create a Google Cloud Composer environment To create an airflow container:
...
3. This will be a good time to go to the bigquery console and cretae new datasets named "footprint_etl" under your project.
   ...
   Make sure that footprint already exists in bigquery_ ETL this data set, if not, create a new one
   
4. Configure the necessary variables according to the guidance of [configuring airflow variables] (#configuring airflow variables) below
5. According to the following [Deploying Airflow DAGs](#deploying-airflow-dags) Deploy DAGs to Cloud Composer Environment.
6. According to the following [here](https://cloud.google.com/composer/docs/how-to/managing/creating#notification)
